...
$(document).ready(function() {
  ...
  */
  ...
  function generate_news(data){
    ...
    on_message("I have searched "+news_data.length+" news articles about "+keywords.toString()+". ");
    var html = "<div class='message_wrapper'>";
    html += "<table class='output message news'>";
  ...
  function generate_social_media(data){
    ...
    var keywords = data.formal_request.keywords;
    html += 'I have analysed '+keywords.toString()+' with 100 social media posts '
          +'and I found that the general opinion is '+data.unformatted_data.general_opinion+' to '+((keywords.length>1)?'them.':'it.');
    html += "<div class='drop_down_arrow'><i class='fas fa-angle-down'></i></div>";
    html += "</div><div class='message output social_media_panel'>The following is the details of the analysis:<br>";
